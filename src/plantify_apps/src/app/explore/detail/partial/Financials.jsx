...
import { Calendar, CalendarDays, TrendingUp, DollarSign, FileText } from 'lucide-react';
      ...
      <div className='rounded-2xl bg-neutral-100 p-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-6'>
          <div className='bg-white rounded-lg p-4 shadow-sm'>
            ...
            <p className='text-2xl font-semibold text-green-600'>${monthlyRevenue.toLocaleString()}</p>
          </div>
          <div className='bg-white rounded-lg p-4 shadow-sm'>
            <h4 className='text-sm text-gray-500 mb-2'>Monthly Expenses</h4>
            <p className='text-2xl font-semibold text-red-600'>${monthlyExpenses.toLocaleString()}</p>
          </div>
          <div className='bg-white rounded-lg p-4 shadow-sm'>
            <h4 className='text-sm text-gray-500 mb-2'>Monthly Profit</h4>
            <p className={`text-2xl font-semibold ${monthlyProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {startup.financialProjections && startup.financialProjections.length > 0 && (
          <div className='bg-white rounded-lg p-4'>
            <h4 className='text-lg font-medium mb-3 flex items-center gap-2'>
              <FileText size={18} className='text-blue-500' />
              Financial Projections
            </h4>
            <p className='text-gray-600 text-sm'>{startup.financialProjections[0]}</p>
          </div>
        )}
      </div>
          ...
          <div className='bg-white rounded-lg p-4'>
            ...
            <div className='space-y-2'>
              ...
              <div className='flex justify-between'>
                ...
                <span className='font-semibold'>${startup.nftPrice || '0'} ckUSDC</span>
              </div>
              <div className='flex justify-between'>
                <span className='text-gray-600'>Company Type:</span>
                <span className='font-semibold'>{startup.companyType || 'N/A'}</span>
              </div>
            </div>
          </div>
          ...
          <div className='bg-white rounded-lg p-4'>
            ...
            <div className='space-y-2'>
              {startup.periodicProfitSharing ? (
                <p className='text-sm text-gray-600'>{startup.periodicProfitSharing}</p>
              ) : (
                <div className='space-y-1 text-sm text-gray-600'>
                  <p>• Profit sharing details will be available after funding</p>
