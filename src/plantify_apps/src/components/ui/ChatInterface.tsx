...
import { X, ArrowUp, TrendingUp, Users, DollarSign, Target } from 'lucide-react';
      ...
      if (
        ...
        const tableMessage: Message = {
          ...
          tableData: {
            ...
            rows: [
              ['Comparable A', '2023', '12x Revenue', startupData?.sector || 'Tech'],
              ['Comparable B', '2024', '10x Revenue', startupData?.sector || 'Tech'],
              ['Comparable C', '2024', '8x Revenue', startupData?.sector || 'Tech'],
            ],
          },
        };
        ...
        const aiResponse: Message = {
          id: (Date.now() + 1).toString(),
          type: 'ai',
          content: `**Risk Assessment for ${startupName}**\n\nRisk Level: ${analysis.riskLevel.toUpperCase()}\n\n**Key Threats:**\n${analysis.threats.map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\n**Mitigation Opportunities:**\n${analysis.opportunities.slice(0, 2).map((o, i) => `${i + 1}. ${o}`).join('\n')}`,
          timestamp: new Date(),
        };

        setMessages(prev => [...prev, aiResponse]);
      }
      else if (lowerMessage.includes('invest') || lowerMessage.includes('recommendation')) {
        if (!startupData) {
          throw new Error('Startup data not available');
        }
        ...
        const recommendationText = {
          strong_buy: '🟢 STRONG BUY - Excellent opportunity with high potential',
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
              ...
              <p className="text-lg font-semibold mb-2">AI Investment Analysis</p>
                ...
                <div className="flex justify-start">
                  <div className="bg-purple-500 border border-gray-200 rounded-2xl p-4 max-w-md shadow-sm space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                        ...
                        <span className="text-xs font-medium text-gray-700">
                          {message.analysisData.investmentRecommendation.replace('_', ' ').toUpperCase()}
                        </span>
                      </div>

                      <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                        <div
                          className={`w-2 h-2 rounded-full ${message.analysisData.riskLevel === 'low'
                            ? 'bg-green-500'
                        ...
                        <span className="text-xs text-gray-700">
                          Risk Level: <span className="font-medium capitalize">{message.analysisData.riskLevel}</span>
                        </span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(message.analysisData.keyMetrics).map(([key, value]) => (
                    ...
                    <div className="pt-2 border-t border-gray-200">
                      <p className="text-xs text-white/80">{message.analysisData.summary}</p>
                    </div>
                  </div>
                </div>
              )}
