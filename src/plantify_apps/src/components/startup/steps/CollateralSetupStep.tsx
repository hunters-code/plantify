...
const CollateralSetupStep: React.FC<CollateralSetupStepProps> = ({ formData, setFormData, errors = {} }) => {
  const [isUploadingBusinessPlan, setIsUploadingBusinessPlan] = useState(false);
  const [isUploadingFinancialProjections, setIsUploadingFinancialProjections] = useState(false);
  const [isUploadingLegalDocuments, setIsUploadingLegalDocuments] = useState(false);
  ...
  const handleBusinessPlanUpload = async (files: File[]) => {
    if (files && files.length > 0) {
      ...
      try {
        ...
        const fileUrl = await uploadFile(file, 'plantify-uploads', 'businessPlan');
  ...
  const handleFinancialProjectionsUpload = async (files: File[]) => {
    if (files && files.length > 0) {
      ...
      try {
        ...
        const fileUrl = await uploadFile(file, 'plantify-uploads', 'financialProjections');
  ...
  const handleLegalDocumentsUpload = async (files: File[]) => {
    if (files && files.length > 0) {
      ...
      try {
        ...
        const fileUrl = await uploadFile(file, 'plantify-uploads', 'legalDocuments');
          ...
          <div className="space-y-2">
            ...
            {formData.businessPlanUrl && !isUploadingBusinessPlan && (
              <div className="mt-2 flex items-center space-x-2">
                ...
                <div>
                  <div className="text-sm font-medium">{formData.businessPlan?.name}</div>
        ...
        <div>
          <div className="space-y-2">
            ...
            {isUploadingFinancialProjections && (
              <div className="flex items-center space-x-2 text-blue-600 mt-2">
                <Loader2 size={16} className="animate-spin" />
                <span className="text-sm">Uploading financial projections...</span>
              </div>
            )}
            
            {formData.financialProjectionsUrl && !isUploadingFinancialProjections && (
              <div className="mt-2 flex items-center space-x-2">
                <div className="p-2 bg-blue-50 rounded-md border border-blue-100">
                  <svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                ...
                <div>
                  <div className="text-sm font-medium">{formData.financialProjectionsFile?.name}</div>
                  <div className="text-xs text-green-600 flex items-center">
                    <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
          ...
          <div className="space-y-2">
            ...
            {formData.legalDocumentsUrl && !isUploadingLegalDocuments && (
              <div className="mt-2 flex items-center space-x-2">
                ...
                <div>
                  <div className="text-sm font-medium">{formData.legalDocuments?.name}</div>
                  <div className="text-xs text-green-600 flex items-center">
                    <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
