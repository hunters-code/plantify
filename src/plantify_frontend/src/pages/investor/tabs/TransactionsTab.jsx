...
export default function TransactionsTab({ onBackToOverview }) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">${transactionData.totalInvested.toLocaleString()}</p>
        </Card>
        
        <Card className="p-4">
          <p className="text-sm text-gray-600 mb-2">Total returns</p>
          <p className="text-2xl font-bold text-gray-900">${transactionData.totalReturns}</p>
        </Card>
        
        <Card className="p-4">
          <p className="text-sm text-gray-600 mb-2">Active investments</p>
          <p className="text-2xl font-bold text-gray-900">{transactionData.activeInvestments}</p>
        </Card>
        
        <Card className="p-4">
          <p className="text-sm text-gray-600 mb-2">Pending withdrawals</p>
          <p className="text-2xl font-bold text-gray-900">${transactionData.pendingWithdrawals}</p>
        </Card>
      </div>
              ...
              <div className="grid grid-cols-3 gap-8">
                <div>
                  <p className="text-sm text-gray-600 mb-1">
                    {transaction.type === 'investment' ? 'NFT purchase' :
                     transaction.type === 'profit' ? 'Monthly returns' :
                     transaction.type === 'topup' ? 'Added funds' :
                     transaction.type === 'fee' ? 'Transaction fee' : transaction.description}
                  </p>
