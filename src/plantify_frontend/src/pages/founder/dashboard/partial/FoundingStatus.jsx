...
export default function FundingStatus({ startupId }) {
  const { 
    startup, 
    totalRaised, 
    fundingGoal, 
        ...
        <div className="w-full">
          <div className="flex gap-2 text-sm mb-2">
            <span className="text-gray-600">
              Overall Progress: <span className="text-orange-500 font-medium">{formatNumber(fundingProgress, 1)}% Funded</span>
            </span>
          </div>
          <ProgressBar
          ...
          <Card className="p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-lg font-semibold">{formatCurrency(Number(collateralInfo.currentAmount))}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Required Amount</p>
                <p className="text-lg font-semibold">{formatCurrency(Number(collateralInfo.requiredAmount))}</p>
              </div>
              <div>
